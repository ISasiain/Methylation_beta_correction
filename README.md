...
1. Generate reference regressions that reflect the different methylation states of each CpG based on the Staaf-Aine β correction estrategy.

2. Etimating purity of the samples from the β values using the refrence regressions generated on the previous step through the newly generated purity estimation algorithm. 

3. Correct β values of the samples whose purty has been estimated using the Staaf-Aine approach. This last step can be carried out refitting the reference regressions to include the new data-points whose purtity has been estimated, or using only the original reference regressions.

![Schedule of the project's workflow_1](./images_in_readme/PIpeline_1.png)
![Schedule of the project's workflow_2](./images_in_readme/PIpeline_2.png)

## SCRIPTS

* MAIN SCRIPTS

    * **ref_regression_caluclator.r**. This script is an adaptation of the Staaf-Aine β correction method, that apart from correcting betas based on the mentioned method, it outputs the reference regressions' parameters to be used by the purity estimation algorithm. This script makes use of the user defined function adjustBeta() from the new_function_correctBetas.r file, which must be located in the same directory for it to run.

    * **purity_estimator.r**. This script estimates sample purity based only on beta values and the regression parameters of a reference data set generated by the previous script. This script makes use of the user defined functions predicting_purity() and purity_value_per_sample() defined in the predicting_purity.r and puroty_coverage.r files, which have to be located in the same directory than the main script in order to be run.

    * **final_beta_correction.r**. This script correct methylation β values of samples whose purity has been estimated using the previous script based on the Staaf-Aine method. The script can applied following two different approaches, refitting the reference regressions using the new datapoints; β values + estimated purity (This appriach makes use of the user defined function adjustBeta() from the new_function_correctBetas.r file, which must be located in the same directory for it to run) or without refitting the reference regressions using the new datapoints, as directly uuses the refrence regressions from the reference dataset to correct betas.
...
* DATA PROCESSING AND PLOTTING SCRIPTS
    ...
    * **split_cross_validation.**. This script splits a reference beta value and purity dataset to be used in a cross-validation approach to test the performance of the designed purity estimation algorithm.
...
* ref_regression_caluclator.r
```bash
Rscript path_to_script/ref_regression_caluclator.r -c [NUM_OF_CORES] -b [INPUT_BETAS.RData] -p [INPUT_PURITIES.RData] -o [OUTPUT_PATH] -n [OUTPUT_NAME] -v [VARIANCE_THRESHOLD]
```
...
```bash
RScript path_to_script/final_beta_corerction.r -c [CORES] -r [refitting: TRUE/FALSE] -R [PATH_TO_REF_REGRESSIONS] -P [REF_COHORT_PURITY] -B [REF_COHORT_BETAS] -p [ESTIMATED_PURITIES] -b [BETAS_TO_CORRECT] -F [CORRECT_CERTAIN_CPGS: TRUE/FALSE] -f [CPGS_TO_CORRECT] -o [OUTPUT_PATH] -n [OUTPUT_NAME]
```
...
Rscript ../../scripts/calculate_regs/ref_regression_caluclator.r -c 7 -b ../original_data/betas_training.RData -p ../original_data/purity_training.RData;
```
...
for dir in $(ls ../../data/slope_filtered/*_BetasTraining.RData);                           
    do fold=$(echo ${dir} | cut -d \/ -f 5 | cut -d _ -f 1);
        ...
        Rscript ../../../../scripts/calculate_regs/ref_regression_caluclator.r -c 37 -b /home/Illumina/Iñaki_Sasiain/08_Cross_validation/data/slope_filtered/${fold}_BetasTraining.RData -p /home/Illumina/Iñaki_Sasiain/08_Cross_validation/data/slope_filtered/${fold}_PurityTraining.RData -o ${fold} -v 0
    done;
```
...
for dir in $(ls ../../data/alpha/*_BetasTraining.RData);                           
    do fold=$(echo ${dir} | cut -d \/ -f 5 | cut -d _ -f 1);
        ...
        Rscript ../../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b /home/Illumina/Iñaki_Sasiain/08_Cross_validation/data/alpha/${fold}_BetasTraining.RData -p /home/Illumina/Iñaki_Sasiain/08_Cross_validation/data/alpha/${fold}_PurityTraining.RData -o ${fold} -v 0
    done;
```
...
Rscript ../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../../data/training_test/betas_training.RData -p ../../data/training_test/purity_training.RData -o LUAC_regs_Training. -v 0.05;
...
Rscript ../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../../data/training_test/betas_training.RData -p ../../data/training_test/purity_training.RData -o LUSC_regs_Training. -v 0.05;
```
...
1. Getting reference raw data

```bash
# Getting reference raw data: BRCA
...
# Getting reference raw data: LUAD
...
# Getting reference raw data: LUSC
...
Rscript ../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../../BRCA_raw/betas.RData -p ../../BRCA_raw/purity.RData -o BRCA_var0.05. -v 0.05;

# Getting regressions when var threshold = 0
cd /home/Illumina/Iñaki_Sasiain/reference/BRCA_regs/var_0;

Rscript ../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../../BRCA_raw/betas.RData -p ../../BRCA_raw/purity.RData -o BRCA_var0. -v 0;
...
Rscript ../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../../LUAD_raw/betas.RData -p ../../LUAD_raw/purity.RData -o LUAD_var0.05. -v 0.05;

# Getting regressions when var threshold = 0
cd /home/Illumina/Iñaki_Sasiain/reference/LUAD_regs/var_0;

Rscript ../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../../LUAD_raw/betas.RData -p ../../LUAD_raw/purity.RData -o LUAD_var0. -v 0;
...
Rscript ../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../../LUSC_raw/betas.RData -p ../../LUSC_raw/purity.RData -o LUSC_var0.05. -v 0.05;

# Getting regressions when var threshold = 0
cd /home/Illumina/Iñaki_Sasiain/reference/LUSC_regs/var_0;

Rscript ../../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../../LUSC_raw/betas.RData -p ../../LUSC_raw/purity.RData -o LUSC_var0. -v 0;
```
...
saveRDS(BRCA_cpgs, file="BRCA1_cpgs.RData");
```
...
1. Copying data used in the last approach. Everything is kept equal except for the reference data used for the correction
```bash
...
2. Filtering reference data to correct. Keep only samples from TNBC
...
Rscript ../../scripts/calculate_regs/ref_regression_caluclator.r -c 40 -b ../ref_betas_and_purities/betas.RData -p ../ref_betas_and_purities/purity.RData -o betas_from_TNBC;
