...
* MAIN SCRIPTS
    ...
    * **new_purity_corrector.r**. This script is an adaptation of the Staaf-Aine β correction method, that apart from correcting betas based on the mentioned method, it outputs the reference regressions' parameters to be used by the purity estimation algorithm. This script makes use of the user defined function adjustBeta() from the new_function_correctBetas.r file, which must be located in the same directory for it to run.
    ...
    * **final_beta_correction.r**. This script correct methylation β values of samples whose purity has been estimated using the previous script based on the Staaf-Aine method. It refits the reference regressions using the new datapoints (β values + estimated purity). This script makes use of the user defined function adjustBeta() from the new_function_correctBetas.r file, which must be located in the same directory for it to run.

    * **final_beta_correction_without_refitting.r**. This script correct methylation β values of samples whose purity has been estimated using the previous script based on the Staaf-Aine method. It does not refit the reference regressions using the new datapoints, as it directly uses the refrence regressions from the reference dataset to correct betas. 
...
Rscript ../../scripts/final_beta_correction/final_beta_correction_without_refitting.r -R ../reference_data/ref_regressions/all_the_cpgs -p ../purity_estimation/GSE148748_est_pur.tsv -b ../data_to_correct/GSE148748_betas.RData -F TRUE -f ../data_to_correct/BRCA1_cpgs.RData -n TNBC_without_refitting;
```
...
Rscript ../../scripts/final_beta_correction/final_beta_correction.r -c 1 -P ../ref_betas_and_purities/purity.RData -B ../ref_betas_and_purities/betas.RData -p ../purity_estimation/GSE148748_est_pur.tsv -b ../data_to_correct/GSE148748_betas.RData -F TRUE -f ../data_to_correct/BRCA1_cpgs.RData -n TNBC_refitting;


#Without refitting the regressions
Rscript ../../scripts/final_beta_correction/final_beta_correction_without_refitting.r -R ../ref_regressions -p ../purity_estimation/GSE148748_est_pur.tsv -b ../data_to_correct/GSE148748_betas.RData -F TRUE -f ../data_to_correct/BRCA1_cpgs.RData -n TNBC_without_refitting;
```
